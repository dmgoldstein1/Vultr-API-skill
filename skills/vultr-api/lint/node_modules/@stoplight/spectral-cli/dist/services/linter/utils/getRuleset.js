...
async function buildSourceContext(rulesetFile) {
    try {
        const input = await fs.promises.readFile(rulesetFile, 'utf8');
        if (path.extname(rulesetFile) === '.json') {
            const parserResult = spectral_parsers_1.Json.parse(input);
            return {
                source: rulesetFile,
                getLocationForJsonPath(jsonPath) {
                    return spectral_parsers_1.Json.getLocationForJsonPath(parserResult, jsonPath);
                },
            };
        }
        const parserResult = spectral_parsers_1.Yaml.parse(input);
        return {
            source: rulesetFile,
            getLocationForJsonPath(jsonPath) {
                return spectral_parsers_1.Yaml.getLocationForJsonPath(parserResult, jsonPath);
            },
        };
    }
    catch {
